<?
	...
	if($mode == "join"){
		...
		echo $shaman->zipTwoAdress($zipData);
		...
		$shaman->insertAffterMemo($setBeen, $SHId);
	}else if($mode == "delMemo"){
		$whereBeen = array(":idx"=>$memoIdx);
		$shaman->deleteAffterMemo($whereBeen, $SHId);
	}else if($mode == "modifyMemo"){
		$setBeen = array($memo);
		$whereBeen = array(":idx"=>$memoIdx);
		$shaman->modifyAffterMemo($setBeen, $whereBeen, $SHId);
