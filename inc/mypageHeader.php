    ...
    <script>
		function goSearch(){
			 if(event.keyCode == 13)
			 {
				...
				location.href = '?com=shaman&lnd=map&searchWord='+searchWord;
			 }
		}
    </script>
</head>
...
<?}?>
    ...
    <div id="scrollMenu" class="sview_content_wrap sv_scroll_menu" style="display:block; position:relative;">
        <ul class="l_style_inline">
            <li <?if($lnd == "modify"){?>class="sv_menu_sel"<?}?>><a href="?com=mypage&lnd=modify">회원정보</a></li>
            <li <?if($lnd == "resList"){?>class="sv_menu_sel"<?}?>><a href="?com=mypage&lnd=resList">예약 확인</a></li>
            <li <?if($lnd == "qList" || $lnd == "qView" || $lnd == "aList" || $lnd == "aView" || $lnd == "bList" || $lnd == "bView"){?>class="sv_menu_sel"<?}?>><a href="?com=mypage&lnd=qList">내가 작성한 글</a></li>
            <li <?if($lnd == "wish"){?>class="sv_menu_sel"<?}?>><a href="?com=mypage&lnd=wish">위시리스트</a></li>
        </ul>
    </div>

<div class="sub_content_wrap" style="width:1024px; ">
