<?
	...
	class Shaman extends ShamanMOL {
		...
		public function shamanInsert($shamanData){
			...
			$this->common->finalMove("lnd","점집이 등록 되었습니다.","user","login");
		}
		...
		/*점집 리스트*/
		public function shamanListMng($page="", $setOrder="", $search=""){
			$returnVal = "";
			$searchQuery = "";
			...
			try{
				...
				$shamanTotalCntResult = parent::shamanTotalList("", $limitQuery);
		...
		public function zipTwoAdress2($bind, $gugun){
			...
			try{
				...
				while (list($key, $val) = each($shamanResult)){
					$selected = "";
					if($shamanResult[$key]["ds_gugun"] == $gugun) $selected = "selected";
					$returnVal .= "<option value=\"".$shamanResult[$key]["ds_gugun"]."\" ".$selected.">".$shamanResult[$key]["ds_gugun"]."</option>";
				}
		...
		public function getSearchSHList($page="", $setOrder="", $search){
			...
			try{
				...
				while (list($key, $val) = each($shamanResult)){
					...
					$SHApply = $shamanResult[$key]["SHApply"];
					...
					*/
					...
					$returnVal .= "
						<li>
							...
							<p class=\"photo_link\">
								<a href=\"?com=shaman&lnd=shamanhome&SHId=".$SHId."\"><img src=\"/images/new.gif\" alt=\"new\" />".$SHName."</a>
							</p>
							<p class=\"photo_score\">
			...
			try{
				...
				while (list($key, $val) = each($shamanResult)){
					...
					if($lastCnt == $loopCnt){
						...
						$onclick = "onclick=\"location.href='?com=shaman&lnd=shamanHome&SHId=".$SHId."'\"";
					}
					...
					$returnVal .= "
					   <div style=\"background:url(".$viewName2.") no-repeat; background-size:cover;\" class=\"sl_shop_wrap\" ".$onclick.">
						...
						<div class=\"sl_shop_title\">
							...
							<button style=\"background:url(".$viewName.") no-repeat; background-size:cover;\" type=\"button\" class=\"shop_photo\" onclick=\"location.href='?com=shaman&lnd=shamanHome&SHId=".$SHId."'\"></button>
		...
		public function shamanHomeInfo($getBeen){
			...
			while (list($key, $val) = each($shamanResult)){
				...
				if($shamanMainImg == "")$shamanMainImg = $shamanImg[0];
		...
		/**점짐 메인 정보**/
		public function shamanHomeInfoDaum($getBeen){
			$shamanResult = parent::modifyShamanInfo($getBeen);
			$returnData = array();
			while (list($key, $val) = each($shamanResult)){
				$returnData["idx"]		= $shamanResult[$key]["idx"];
		...
		/**점짐 메인 정보**/
		public function shamanHomeInfoM($getBeen){
			$shamanResult = parent::modifyShamanInfo($getBeen);
			$returnData = array();
			while (list($key, $val) = each($shamanResult)){
				$returnData["idx"]		= $shamanResult[$key]["idx"];
		...
		/**점짐 메인 정보**/
		public function shamanHomeInfoMDaum($getBeen){
			$shamanResult = parent::modifyShamanInfo($getBeen);
			$returnData = array();
			while (list($key, $val) = each($shamanResult)){
				$returnData["idx"]		= $shamanResult[$key]["idx"];
				...
				while (list($key, $val) = each($shamanResult)){
					...
					if($notIdx != $shamanResult[$key]["SHIdx"]){
						...
						if($wishCnt > 0){
							...
							if($_SESSION["USER_ID"] != ""){
								$heartImg = "<img class=\"sc_heart\" src=\"/images/heart_off.png\" alt=\"\" onclick=\"setListWishH('".$shamanResult[$key]["SHIdx"]."');\"/>";
							}else{
								$heartImg = "<img class=\"sc_heart\" src=\"/images/heart_off.png\" alt=\"\"/>";
							}
							//$heartImg = "<img class=\"sc_heart\" src=\"/images/heart_off.png\" alt=\"\" />";
				...
				while (list($key, $val) = each($shamanResult)){
					if($notIdx != $shamanResult[$key]["SHIdx"]){
						...
						if($wishCnt > 0){
							...
							if($_SESSION["USER_ID"] != ""){
								$heartImg = "<img class=\"sc_heart\" src=\"/images/mobile/ic_heart.png\" alt=\"\" onclick=\"setListWishH('".$shamanResult[$key]["SHIdx"]."');\"/>";
							}else{
					...
					if($saveName2 == ""){
						...
						$returnVal .= "
							...
							<li>
								".$heartImg."
								<div class=\"sc_photo_wrap\">
									<a href=\"?com=shaman&lnd=shamanHome&SHId=".$SHId."\"><img src=\"".$viewName2."\" alt=\"\" /></a>
									<div class=\"sc_money\">
										<span>￦</span>".number_format($prdPrice)."
									</div>
								</div>
